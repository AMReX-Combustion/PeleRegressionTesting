...
test_configuration() {
  ...
  if [ "${COMPILER_NAME}" == 'clang' ]; then
    ...
    CMAKE_CONFIGURE_ARGS="-DPELELM_ENABLE_CPPCHECK:BOOL=ON -DPELELM_ENABLE_CLANG_TIDY:BOOL=ON -DPELELM_ENABLE_SANITIZE_FOR_TESTS:BOOL=ON -DPELELM_ENABLE_FPE_TRAP_FOR_TESTS:BOOL=OFF ${CMAKE_CONFIGURE_ARGS}"
  ...
  CMAKE_CONFIGURE_ARGS="-DCMAKE_CXX_COMPILER=${CXX_COMPILER} -DCMAKE_C_COMPILER=${C_COMPILER} -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DPELELM_ENABLE_MPI:BOOL=ON -DPYTHON_EXECUTABLE=${PYTHON_EXE} -DPELELM_ENABLE_FCOMPARE_FOR_TESTS:BOOL=ON ${CMAKE_CONFIGURE_ARGS}"
