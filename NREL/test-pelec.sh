...
test_configuration() {
  ...
  if [ "${MACHINE_NAME}" == 'eagle' ]; then
    ...
    CMAKE_CONFIGURE_ARGS="-DPELEC_ENABLE_CUDA:BOOL=ON -DAMReX_CUDA_ARCH:STRING=7.0 -DCMAKE_CUDA_ARCHITECTURES:STRING=70 -DGPUS_PER_NODE:STRING=2 -DGPUS_PER_SOCKET:STRING=1 ${CMAKE_CONFIGURE_ARGS}"
  ...
  if [ "${COMPILER_NAME}" == 'clang' ]; then
    ...
    #CMAKE_CONFIGURE_ARGS="-DPELEC_ENABLE_CPPCHECK:BOOL=OFF -DPELEC_ENABLE_CLANG_TIDY:BOOL=OFF ${CMAKE_CONFIGURE_ARGS}"
    CMAKE_CONFIGURE_ARGS="-DPELEC_ENABLE_SANITIZE_FOR_TESTS:BOOL=ON ${CMAKE_CONFIGURE_ARGS}"
    CTEST_ARGS="-DRUN_CODE_ANALYSIS:BOOL=FALSE ${CTEST_ARGS}"
  fi
