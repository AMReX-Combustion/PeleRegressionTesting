...
test_configuration() {
  ...
  if [ "${COMPILER_NAME}" == 'clang' ]; then
    ...
    CMAKE_CONFIGURE_ARGS="-DPELEC_ENABLE_CPPCHECK:BOOL=ON -DPELEC_ENABLE_CLANG_TIDY:BOOL=ON -DPELEC_ENABLE_SANITIZE_FOR_TESTS:BOOL=ON -DPELEC_ENABLE_FPE_TRAP_FOR_TESTS:BOOL=OFF ${CMAKE_CONFIGURE_ARGS}"
    CTEST_ARGS="-DRUN_CODE_ANALYSIS:BOOL=TRUE ${CTEST_ARGS}"
  elif [ "${COMPILER_NAME}" == 'intel' ]; then
    CMAKE_CONFIGURE_ARGS="-DPELEC_ENABLE_FPE_TRAP_FOR_TESTS:BOOL=OFF ${CMAKE_CONFIGURE_ARGS}"
  else
    CMAKE_CONFIGURE_ARGS="-DPELEC_ENABLE_FPE_TRAP_FOR_TESTS:BOOL=ON ${CMAKE_CONFIGURE_ARGS}"
  fi
  ...
  CMAKE_CONFIGURE_ARGS="-DCMAKE_CXX_COMPILER=${CXX_COMPILER} -DCMAKE_C_COMPILER=${C_COMPILER} -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DPELEC_ENABLE_MPI:BOOL=ON -DPYTHON_EXECUTABLE=${PYTHON_EXE} -DPELEC_ENABLE_FCOMPARE_FOR_TESTS:BOOL=ON ${CMAKE_CONFIGURE_ARGS}"
...
EOL
  ...
  (set -x; rsync -avm --include="*/" --include="plt00010**" --exclude="*" ${PELEC_DIR}/build/Exec/RegTests/ ${PELEC_TESTING_ROOT_DIR}/temp_golds/${ID_FILE}/)
...
main() {
  ...
  #  CONFIGURATIONS[0]='clang:12.0.0-apple:false:false:masa;sundials'
