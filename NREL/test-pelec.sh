...
test_configuration() {
  ...
  if [ "${COMPILER_NAME}" == 'clang' ]; then
    ...
    CMAKE_CONFIGURE_ARGS="-DPELEC_ENABLE_CPPCHECK:BOOL=ON -DPELEC_ENABLE_CLANG_TIDY:BOOL=ON -DPELEC_ENABLE_SANITIZE_FOR_TESTS:BOOL=ON -DPELEC_ENABLE_FPE_TRAP_FOR_TESTS:BOOL=ON ${CMAKE_CONFIGURE_ARGS}"
    CTEST_ARGS="-DRUN_CODE_ANALYSIS:BOOL=TRUE ${CTEST_ARGS}"
  elif [ "${COMPILER_NAME}" == 'intel' ]; then
    #export CXXFLAGS="-fp-model consistent"
