...
test_configuration() {
  ...
  cmd "spack install ${MPI_ID} %${COMPILER_ID} ${GENERAL_CONSTRAINTS}"
  ...
  if [ "${MACHINE_NAME}" == 'rhodes' ] && [ "${COMPILER_ID}" == 'gcc@7.4.0' ]; then
    ...
    cmd "cppcheck --enable=warning,performance,portability,information,unusedFunction,missingInclude --quiet -j 16 --output-file=${LOGS_DIR}/pelec-static-analysis.txt -I ${PELEC_DIR}/Submodules/AMReX/Src/Base ${PELEC_DIR}/Submodules/AMReX/Src/Amr ${PELEC_DIR}/Submodules/AMReX/Src/AmrCore ${PELEC_DIR}/Submodules/AMReX/Src/Base ${PELEC_DIR}/Submodules/AMReX/Src/F_Interfaces/AmrCore ${PELEC_DIR}/Submodules/AMReX/Src/Boundary ${PELEC_DIR}/Submodules/AMReX/Tools/C_scripts ${PELEC_DIR}/Submodules/AMReX/Src/F_Interfaces/Base ${PELEC_DIR}/Submodules/AMReX/Src/EB ${PELEC_DIR}/Submodules/PelePhysics/Transport ${PELEC_DIR}/Submodules/PelePhysics/Reactions ${PELEC_DIR}/Submodules/PelePhysics/Support/Fuego/Evaluation ${PELEC_DIR}/Submodules/PelePhysics/Support/Fuego/Mechanism ${PELEC_DIR}/Source ${PELEC_DIR}/Source/param_includes || true"
  ...
  CMAKE_CONFIGURE_ARGS="-DPYTHON_EXECUTABLE=${PYTHON_EXE} -DENABLE_VERIFICATION:BOOL=ON -DTEST_WITH_FCOMPARE:BOOL=ON -DTEST_WITH_FEXTREMA:BOOL=ON ${CMAKE_CONFIGURE_ARGS}"
...
EOL
  ...
  (set -x; rsync -avm --include="*/" --include="*ext.gold" --exclude="*" ${PELEC_DIR}/build/Testing/test_files/ ${PELEC_TESTING_ROOT_DIR}/temp_golds/${ID_FILE}/)
  # This only works on Linux
...
main() {
  ...
  if [ ! -d "${PELEC_TESTING_ROOT_DIR}" ]; then
    ...
    # Checkout PeleC
    printf "\nCloning PeleC repo...\n"
    cmd "git clone --recursive -b development https://github.com/AMReX-Combustion/PeleC.git ${PELEC_DIR}"
